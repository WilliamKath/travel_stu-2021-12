...
public class LoginController {
  ...
  public ResMsg signIn(@RequestBody LoginVo vo) {
    ...
    if (StringUtils.isNotBlank(vo.getUsername()) && StringUtils.isNotBlank(vo.getPassword())) {
      ...
      if (dbuser !=null && userService.findActiveUserByUserName(dbuser.getUsername()).getStatus().equals("Y")==true) {
        if (!userService.validUserPwd(dbuser.getUsername(), dbuser.getPassword()))
        {
          resMsg.setErrcode("0");
